...
bash 执行 seam 分为 Service Definition（[dsh-shell](../../packages/shell/shell)，`ctx.shell`）、Service Provider（[dsh-bash-local](../../packages/shell/bash-local)、[dsh-bash-sandbox](../../packages/shell/bash-sandbox) 与 [dsh-shell-slark-remote](../../packages/shell/shell-slark-remote)）和 Consumer（[dsh-tool-bash](../../packages/shell/tool-bash)，即 `bash` schema）。通用后台任务的 job id、所有权与控制位于 [jobs.md](jobs.zh.md)；本 seam 返回一个不含任务概念的进程句柄。本地原始进程组机制封装在[子进程 seam](subprocess.zh.md)之后，而 Slark Provider 通过可恢复的不透明代理句柄寻址 Device Agent 进程。
