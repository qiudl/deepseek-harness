...
flowchart LR
  ...
  pkg_slark_identity["slark-identity"]
  svc_slarkIdentity["ctx.slarkIdentity<br/>Slark runtime authority scope"]
  ...
  pkg_shell_slark_remote["shell-slark-remote"]
  ...
  pkg_fs_slark_remote["fs-slark-remote"]
  pkg_fs_observation_policy["fs-observation-policy"]
  pkg_slark_device_client["slark-device-client"]
  svc_slarkDevice["ctx.slarkDevice<br/>Slark local-device task transport"]
  ...
  pkg_shell_slark_remote --> svc_shell
  pkg_skill --> svc_skills
  pkg_skill_badge --> svc_skills
  pkg_skill_filesystem --> svc_skills
  pkg_slark_device_client --> svc_slarkDevice
  pkg_slark_identity --> svc_slarkIdentity
  ...
  svc_slarkDevice --> pkg_fs_slark_remote
...
| `ctx.slarkIdentity` | `core` | [`slark-identity`](../packages/host/slark-identity) | - | - | - | 校验当前由 Edge 签发的 authority，并在 Agent 与工具工作中携带活跃的 DSH Session，随后远程 Device 操作才解析其 fences。 |
...
| `ctx.shell` | `seam` | [`shell`](../packages/shell/shell) | [`bash-local`](../packages/shell/bash-local), [`bash-sandbox`](../packages/shell/bash-sandbox), [`pwsh-local`](../packages/shell/pwsh-local), [`shell-slark-remote`](../packages/shell/shell-slark-remote) | [`tool-bash`](../packages/shell/tool-bash), [`tool-pwsh`](../packages/shell/tool-pwsh), [`hooks-claude-code`](../packages/hooks/hooks-claude-code), [`hooks-codex`](../packages/hooks/hooks-codex) | - | 面向模型的 shell 工具和钩子桥接消费此 seam；沙箱、远程或 PowerShell 执行器可以替换 bash-local，而无需改动这些消费方。 |
...
| `ctx.fs` | `seam` | [`fs`](../packages/fs/fs) | [`fs-local`](../packages/fs/fs-local), [`fs-sandbox`](../packages/fs/fs-sandbox), [`fs-e2b`](../packages/e2b/fs-e2b), [`fs-slark-remote`](../packages/fs/fs-slark-remote) | [`tool-fs`](../packages/fs/tool-fs) | [`fs-observation-policy`](../packages/fs/fs-observation-policy) | tool-fs 通过 ctx.fs 执行读取／写入／编辑；fs-sandbox 按共享沙箱模式限制变更；fs-observation-policy 通过 fs/* 事件门禁贡献基于观测状态的检查。 |
| `ctx.slarkDevice` | `seam` | [`slark-device-client`](../packages/slark/device-client) | - | [`fs-slark-remote`](../packages/fs/fs-slark-remote) | - | 负责带身份认证的持久任务提交、轮询、重试、取消和有界输出重组；远程文件系统提供方把中立的文件操作转换到该传输层。 |
