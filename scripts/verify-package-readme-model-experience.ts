...
const SENTENCE_MODEL_EXPERIENCE: Readonly<Record<string, SentenceContract>> = {
  ...
  'packages/shell/shell-slark-remote': { kind: 'indirect', reason: 'The remote executor backend delegates model rendering to dsh-tool-bash.' },
  ...
  'packages/slark/collaboration-network': { kind: 'indirect', reason: 'The worker delivers verified input to a selected formal Agent; that Agent preset owns the resulting model context.' },
  ...
  'packages/fs/fs-slark-remote': { kind: 'indirect', reason: 'The provider backend delegates model rendering to dsh-tool-fs.' },
  'packages/slark/device-client': { kind: 'indirect', reason: 'Capability providers own every model-visible schema and result rendered over the transport.' },
  ...
  'packages/host/slark-identity': { kind: 'none', reason: 'The authority adapter keeps Slark identity outside model requests and session logs.' },
