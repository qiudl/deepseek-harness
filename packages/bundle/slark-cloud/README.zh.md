...
| `SLARK_DSH_AUTHORITY_FILE` | Edge 私有 authority 文档的绝对路径 |
| `SLARK_DSH_WORKSPACE_HANDLE` | 身份、文件系统和 Shell Provider 共用的不透明 handle |
...
- 首发版本在一次 Runtime Cell 组合中固定一个 Workspace Grant。切换 workspace 需要重新组合 cell。
- 本组合包只提供配置和静态证明；Edge／Cell 进程隔离、资源限额、健康检查和 draining 属于 Slark 部署任务。
