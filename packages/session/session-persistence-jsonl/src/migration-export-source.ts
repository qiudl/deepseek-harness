...
export class FileJsonlMigrationExportSource implements MigrationExportSource {
  ...
  private async scan(signal?: AbortSignal): Promise<Stored[]> {
    ...
    const migrationSignal = signal ?? new AbortController().signal
    for (const row of rows) {
      const children = rows.filter(candidate =>
        candidate.header.origin === 'subagent' && candidate.header.parentSession === row.header.id)
      const facts: SessionFormatJsonObject[] = []
      const validations: Array<() => Promise<void>> = []
      for (const compression of ['none', 'zstd'] as const) {
        const sources = children.filter(candidate =>
          compression === 'zstd' ? candidate.path.endsWith('.zstd') : !candidate.path.endsWith('.zstd'))
        if (sources.length === 0) continue
        const prepared = await prepareCatalogFacts(row.header.id, sources, compression, migrationSignal)
        facts.push(...prepared.facts)
        validations.push(prepared.validate)
      }
      complete.push(await this.readLog(row.path, row.path.endsWith('.zstd'), signal, facts))
      for (const validate of validations) await validate()
    }
    return complete.sort((left, right) => left.header.id.localeCompare(right.header.id, 'en'))
  }
  ...
  private async readLog(
    ...
    zstd: boolean,
    signal?: AbortSignal,
    childFacts: readonly SessionFormatJsonObject[] = [],
  ): Promise<Stored> {
    ...
    try {
      ...
      try {
        ...
        const upgraded = this.upgradeForeignLog(plaintext, childFacts)
  ...
  private upgradeForeignLog(
    ...
    const header = JSON.stringify(
      catalog.encodeCurrentHeader(current.header, current.inheritedEventCount),
    )
    const rows = current.events.map(event => JSON.stringify(catalog.encodeCurrentEvent(event)))
