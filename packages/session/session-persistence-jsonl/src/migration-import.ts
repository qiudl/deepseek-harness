...
import { SESSION_FORMAT_VERSION, SessionLogOffset, type SessionEvent, type SessionHeader } from '@deepseek-ai/dsh-session'
...
import { LEASE_FILENAME } from './lease.js'
...
function sessionInheritedEventCount(
  meta: SessionHeader,
  events: readonly SessionEvent[],
): ReturnType<typeof SessionLogOffset> {
  let cut: ReturnType<typeof SessionLogOffset> | undefined
  ...
  if (meta.isSeeded && cut === undefined) {
    throw new Error('corrupt session log: seeded header lacks an inherited end-seed marker')
  }
  if (!meta.isSeeded && cut !== undefined) {
    throw new Error('corrupt session log: unseeded header contains an inherited end-seed marker')
  }
...
export class FileOwnerJsonlMigrationGenerationTarget implements MigrationImportTarget {
  ...
  async semanticRecords(generation: number): Promise<readonly MigrationSemanticRecord[]> {
    ...
    for (const project of await readdir(generationRoot, { withFileTypes: true })) {
      ...
      for (const session of await readdir(projectPath, { withFileTypes: true })) {
        ...
        if (JSON.stringify(names) !== JSON.stringify(['migration-records.json', generationLogFilename(SESSION_FORMAT_VERSION, 'none')])) {
          throw new Error('migration_generation_unsafe')
        }
  ...
  private assertContained(root: string, path: string): void {
    ...
    if (!within || within === '..' || within.startsWith(`..${sep}`) || basename(path) !== generationLogFilename(SESSION_FORMAT_VERSION, 'none')) {
      throw new Error('migration_generation_unsafe')
    }
  }
