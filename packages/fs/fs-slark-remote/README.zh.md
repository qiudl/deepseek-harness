...
- `writeText` 与 `editText` 保留基础 `FileSystem` 约定：带防护与无条件变更仍是两种不同语义。每次变更取得新的副作用围栏，而 Gateway 的含糊重试会复用同一逻辑 Device Task。
...
- 搜索工具仍由子进程支持。云端组合必须挂载匹配的远程 shell 提供方后才能暴露 `glob` 或 `grep`；本文件系统包不会新增搜索 RPC。
