...
Global panels occupy the root-scoped `main` keyed slot; `conversation` is the reserved key for the Conversation. `ctx.layout.selectPanel(id)` selects a registered panel, and `null` selects the Conversation without changing the current Session. Selecting a global panel hides but does not unmount the Conversation, so component-local drafts and scroll state survive the switch. No global panel is registered by the shipped composition.
