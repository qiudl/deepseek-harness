...
A remote shell supplies `fetch`, `openStream`, and `loadBundle` through `__DSH_TRANSPORT__` and sets `remoteHost: true`, `ownsHost: false`. The remote marker makes `ctx.connection.isLoopback` false even if the shell page runs on localhost or outside a browser. The shell must authenticate the selected Host and verify its boot injections and bundle bytes before supplying them; this package does not grant those guarantees.
