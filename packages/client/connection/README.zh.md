...
远程 shell 通过 `__DSH_TRANSPORT__` 提供 `fetch`、`openStream` 和 `loadBundle`，并设置 `remoteHost: true`、`ownsHost: false`。即使 shell 页面运行在 localhost 或非浏览器环境中，远程标记也使 `ctx.connection.isLoopback` 为 false。shell 必须先认证所选 Host，并验证其启动注入与 bundle 字节；本包不提供这些保证。
