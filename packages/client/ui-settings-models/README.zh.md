...
在 Slark Desktop 的个人账号视图中，模型页面可以检查本机旧版 DSH 的脱敏提供方信息。用户经 Desktop Main 的原生确认框逐个认领；页面只接收候选项元数据、来源摘要与操作回执。未完成的认领提供状态查询、重试和恢复入口。普通浏览器不显示认领卡片。新增或替换密钥仍使用现有 API 密钥编辑器。
