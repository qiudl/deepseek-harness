...
`profile.remote_ui_read` 将 `session/list`、`session/page` 或 `session/modelCatalog` 三种读取绑定到同一有效视图租约。请求只能携带有界的 `args` 对象，不能指定 URL、Profile 路径、cookie 或 worker token。结果受 64 KiB 控制帧限制；大页和流式事件需要独立传输。Host 仅在安装 worker 执行器后发布此 capability。
...
- **操作集合有明确上限**——版本 1 解析 `host.inspect`、账号与本地专用 Profile provisioning/restore/open、Profile status/lease-close、迁移导出 begin/read、扩展命令、远程 Session 命令、三种远程 UI 读取与通用错误。远程方法在 Host 执行器发布 capability 前仍不可用；environment、attachment 和 upgrade 操作需要显式扩展协议。
