...
`profile.remote_ui_read` binds `boot/injections`, `asset/read`, `session/list`, `session/page`, or `session/modelCatalog` to the same live view lease. The boot read requires empty `args` and returns the current Profile's structured startup rows. `asset/read` accepts only a plugin URL and byte offset; the Host worker serves chunks only for URLs currently named by those startup rows. No request can name a Profile path, cookie, worker token, or arbitrary URL. Each result remains below the 64 KiB control-frame limit; large pages and streaming events need a separate transport. The Host advertises this capability only with a worker executor installed.
