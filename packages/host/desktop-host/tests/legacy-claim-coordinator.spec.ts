...
describe('Host-internal legacy claim transaction order', () => {
  it('exposes only an owned durable receipt and refuses an unreserved retry', async () => {
    ...
    expect(() => { void state.coordinator.retry(state.claim) }).toThrow(/conflict/u)
    ...
    expect(() => { state.coordinator.status({ candidateId, authorizeAccountProfile: () => otherProfileId }) })
      .toThrow(/conflict/u)
    expect(() => { void state.coordinator.retry({ ...state.claim, operationId: '2a2ec924-9005-4bcd-ae53-aa8f4f74fcf3' }) })
      .toThrow(/conflict/u)
    expect(() => { void state.coordinator.retry({ ...state.claim, expectedSourceDigest: 'b'.repeat(64) }) })
      .toThrow(/conflict/u)
