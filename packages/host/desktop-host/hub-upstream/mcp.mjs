// Generated from pinned Desktop Hub mcp.ts; see UPSTREAM.json and LICENSE.
    ...
    for (const [name, raw] of Object.entries(mcpServers)) {
        ...
        if (typeof s.command === 'string') {
            ...
            if (Array.isArray(s.args))
                base.args = s.args.map(String);
            if (s.env && typeof s.env === 'object')
                base.env = Object.fromEntries(Object.entries(s.env).map(([k, v]) => [k, String(v)]));
            ...
            if (type === 'sse')
                warnings.push(`「${name}」type=sse：DSH 仅支持 streamable-http，将按 HTTP 处理（需确认端点兼容）`);
            ...
            if (s.headers && typeof s.headers === 'object') {
                base.headers = Object.fromEntries(Object.entries(s.headers).map(([k, v]) => [k, String(v)]));
            }
        }
        servers.push(base);
    }
...
export function convertToRows(servers) {
    return servers.map((s) => {
        const config = { serverName: s.name, transport: s.transport };
        if (s.command)
 ...
 */
export function renderRowsYaml(rows) {
    const text = stringify([{ insert: rows }]);
    return text.replace(/^(\s*[A-Za-z0-9_.-]+):\s*(['"])?\$\{([A-Za-z_][A-Za-z0-9_]*)\}\2\s*$/gm, '$1: !!js process.env.$3');
}
/** JSON 文本 → 转换预览（YAML），env/headers 中 ${VAR} 转为 DSH 的 !!js process.env.VAR */
export function convertJsonToYaml(text) {
    try {
        const { servers, warnings } = parseMcpJson(text);
        if (servers.length === 0)
            return { ok: false, error: '没有可转换的服务器', warnings };
...
function unwrap(v) {
    if (v && typeof v === 'object' && !Array.isArray(v)) {
        ...
        if (typeof o.type === 'string' && 'value' in o) {
            ...
            if (o.tag === JS_TAG)
                return { $js: String(o.value) };
            return unwrap(o.value);
        }
        if (Array.isArray(o.items)) {
            const first = o.items[0];
            const isMap = first && typeof first === 'object' && first.key !== undefined;
            if (isMap) {
                const out = {};
                for (const pair of o.items) {
                    const k = pair.key && typeof pair.key === 'object' && 'value' in pair.key ? pair.key.value : pair.key;
                    out[String(k)] = unwrap(pair.value);
                }
                return out;
            }
            return o.items.map(unwrap); // Seq
        }
    }
    return v;
}
/** 从 profile cordis.patch.yml 提取现有 MCP 服务器 */
export function extractMcpServers(patchText) {
    const doc = parseDocument(patchText);
    if (doc.errors.length > 0)
        throw new Error(`patch 解析失败: ${doc.errors[0].message}`);
    const rows = [];
...
export function replaceMcpRows(patchText, rows) {
    const doc = parseDocument(patchText);
    if (doc.errors.length > 0)
        throw new Error(`patch 解析失败: ${doc.errors[0].message}`);
    const entries = doc.contents?.items ?? [];
    ...
    if (rows.length > 0) {
        ...
        const freshEntries = (fresh.contents?.items ?? []);
        if (!targetInsert) {
            if (!doc.contents)
...
function findRowLocation(doc, id) {
...
function appendRowNode(doc, row) {
    for (const entry of patchEntries(doc)) {
        const node = entry;
        if (!node?.items)
            continue;
    ...
    const freshEntry = fresh.contents?.items?.[0];
    if (!freshEntry)
        throw new Error(`MCP 行序列化失败: ${row.id}`);
    doc.contents.items?.push(freshEntry);
}
/** 合并写入：按行 id 就地更新/追加草稿行。
 * AST 行级操作：未编辑的 MCP 行（含 !!js 动态值）与其他插件行/注释原样保留。 */
export function mergeMcpRows(patchText, rows) {
    const doc = parseDocument(patchText);
    if (doc.errors.length > 0)
        throw new Error(`patch 解析失败: ${doc.errors[0].message}`);
    for (const row of rows) {
        const loc = findRowLocation(doc, row.id);
        if (loc)
            loc.seq[loc.index] = newRowNode(row);
        else
            appendRowNode(doc, row);
    }
    return doc.toString();
}
/** 更新已有 MCP 行，保留同一 id 以支持 UI 编辑（行级替换，其他行 AST 原样）。 */
export function updateMcpRow(patchText, row) {
    const doc = parseDocument(patchText);
    if (doc.errors.length > 0)
        throw new Error(`patch 解析失败: ${doc.errors[0].message}`);
    const loc = findRowLocation(doc, row.id);
    if (!loc)
        throw new Error(`MCP 服务器不存在: ${row.id}`);
    loc.seq[loc.index] = newRowNode(row);
    return doc.toString();
}
/** 删除已有 MCP 行（行级删除，其余行 AST 原样）；允许删除最后一个服务器。 */
export function deleteMcpRow(patchText, id) {
    const doc = parseDocument(patchText);
    if (doc.errors.length > 0)
        throw new Error(`patch 解析失败: ${doc.errors[0].message}`);
    const loc = findRowLocation(doc, id);
    if (!loc)
 ...
 *   与 dsh 自身写入器的重试策略对齐。 */
export function atomicWriteWithBackup(file, content, backupsDir) {
