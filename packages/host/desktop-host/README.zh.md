...
- **旧模型盘点只读**——声明旧来源已静止的 macOS Host 会发布 `profile.model_claim_inventory`、`profile.model_claim_confirm` 和 `profile.model_claim_apply`。盘点只返回脱敏候选元数据。确认重验 Account 租约、来源摘要和凭据是否存在，再签发只在当前连接有效、60 秒内只能使用一次的授权。写入在认领事务前消费该授权。Windows 在原生来源盘点完成前不发布这些能力。
