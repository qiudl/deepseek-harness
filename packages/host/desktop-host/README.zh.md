...
- **账号认领授权独立于普通视图访问**——Host 只能凭有效的 Main 持有租约，为通过 `profile.ensure` 验证过 Account 令牌的同一连接解析 Account Profile 认领目标。仅靠本机保管库恢复、离线恢复、本地 Profile、连接撤销和过期租约都不能授权认领。认领控制接口尚未调用这项检查。
