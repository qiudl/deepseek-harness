...
Windows 目录安全证据通过 ConvertSidToStringSidW 从二进制安全描述符解码所有者和每个 ACE 的受托人 SID，因此已知 SID 保留规范数字形式，而不是 SDDL 别名字符串（内置 Administrator 账户在 SDDL 文本中序列化为 "LA"，但仍必须与调用方的完整 SID 匹配）。访问掩码按原样读取；除允许和拒绝之外的其他 ACE 类型会使检查失败。解码不会授予私有存储访问权：私有文件仍要求精确的、受保护的三主体文件完全控制 DACL。
