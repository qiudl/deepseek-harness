...
- **旧数据认领账本是 Host 内部基础能力**——状态机校验不含密钥的预留、提交和已验证恢复事件。Unix 用户私有及 Windows SID 私有适配器会先原子替换有大小上限的快照，再发布状态变化。重启后仍能识别未决预留；替换结果不确定时，重载前拒绝后续写入并将受影响 Profile 标记为未决。向用户提供认领前，仍需接入 worker 管控和目标凭据写入；损坏的全局账本不能让无关 DSH Profile 停摆。
