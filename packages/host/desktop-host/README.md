...
Windows installation identity preparation accepts separate private-storage and environment roots. It creates the ordinary parent directory first, requires the environment root to be a SHA-256 namespace directly below the storage root's `environments` directory, then creates and verifies every private directory from the storage root downward in order.

The Windows client uses a private file-backed Worker to attest the connected pipe server before sending Host frames. Its Bun Main cancellation adapter is supplied by this package through the verified client artifact; callers provide the release-pinned Worker and exactly one peer identity mode. Authenticode mode pins publisher thumbprints, while Store mode pins package family names and requires the executable to remain inside the package's protected install root; both modes also pin the executable digest. Worker exit must be confirmed before its transferred thread handle is closed.
...
The macOS startup composition validates owner-only non-symlink roots, starts exactly one Host, checks the Node executable and fixed DSH entrypoint independently, verifies the Account public keyring against the embedding release's SHA-256 pin, performs native peer PID/executable/code-signature attestation, and publishes the exact secret-free `~/.dsh/host/registration.v1.json` discovery record. After acquiring exclusive Host ownership, a runtime upgrade may atomically refresh only that record's executable signature digest; every installation, key, endpoint, and socket field must remain identical. Profile workers inherit no ambient environment. Windows passes the explicit Profile environment through a bounded private file-descriptor pipe because packaged child processes may discard custom environment values; the bootstrap clears ambient values before importing the fixed DSH entrypoint. The Host verifies the child owns its reported loopback listener, exchanges its one-use launch token for a signed cookie, confirms unauthenticated `/` is 401 and authenticated `/` is 200, then discards the token.
...
See the [single Host control protocol Agent Note](../../../.agents/notes/implemented/architecture/2026-09-02-single-host-control-protocol.md) and [packaged Windows Profile configuration Agent Note](../../../.agents/notes/implemented/bug-fix/2026-09-15-windows-packaged-profile-worker-configuration.md).
