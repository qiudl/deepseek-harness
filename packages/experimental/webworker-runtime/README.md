...
- **`lib/client.js` (page half)** — startup has two independent stages. `chooseWorkerHostSource({ image?, fixtureManifest? })` optionally owns the boot barrier and fixture manifest: without `preview-fixture` it waits at the source chooser, while a valid query selects directly; either path returns ordered overlays. `connectWorkerHost(worker, { image?, overlays?, localProfile? })` remains the public base-runtime connector; callers that skip the chooser get an empty overlay list. A selected local profile is unlocked on the page with a resident passkey and required user verification, then its non-extractable AES-GCM key is structured-cloned directly to the same-origin Worker. The Worker requests persistent storage and an exclusive profile Web Lock, encrypts only `home/` and `workspace/` records in IndexedDB, applies them after the immutable image/overlays and returns an explicit `durable` or `session_only` boot state. Environment and UUID profile identifiers bind the passkey wrapper, profile key-check and every entry ciphertext. `apps/web` invokes both startup stages and supplies its statically bundled Worker. The boot payload also delivers the structured index-injection table, and `applyIndexInjections` executes it before the shell entry runs. Script preload rows are advisory and skipped because `/plugins` resources resolve only through the tunnel; `loadBundle` fetches each combo on first demand, embeds its tunnel-only source map as a Base64 data URL, and executes the script as a Blob. The tunnel also exposes fetch-shaped transport and the API client.
...
- **Encrypted local-profile primitives remain inactive in the preview page**: the package has no origin-owned profile registry, recovery-passphrase/export-pack flow, or user-facing first-run/migration UI. `apps/web/src/preview.ts` deliberately supplies no `localProfile`, so this foundation cannot silently create or take over browser data. A post-boot IndexedDB write failure stops mirroring and preserves the live in-memory session; the page receives no live downgrade notification.
