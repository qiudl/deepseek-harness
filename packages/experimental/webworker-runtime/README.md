...
- **`lib/client.js` (page half)** — startup has two independent stages. `chooseWorkerHostSource({ image?, fixtureManifest? })` optionally owns the boot barrier and fixture manifest: without `preview-fixture` it waits at the source chooser, while a valid query selects directly; either path returns ordered overlays. `connectWorkerHost(worker, { image?, overlays?, localProfile? })` remains the public base-runtime connector; callers that skip the chooser get an empty overlay list. Enrollment wraps one random data key with both a user-verified resident-passkey PRF and a versioned recovery passphrase, and the origin registry stores only strict wrapper metadata. The registry, VFS key check, and encrypted entries share one versioned IndexedDB schema; new-profile initialization commits its registry record and key check together. Recovery export adds an authenticated AES-GCM layer around a consistent full-profile snapshot; recovery import validates the independently selected environment and commits all three stores atomically without overwriting an existing profile. A selected local profile is unlocked on the page, then its non-extractable AES-GCM key is structured-cloned directly to the same-origin Worker. The Worker requests persistent storage and an exclusive profile Web Lock, encrypts only `home/` and `workspace/` records in IndexedDB, applies them after the immutable image/overlays and returns an explicit `durable` or `session_only` boot state. Environment and UUID profile identifiers cryptographically bind both wrappers, profile key-check and every entry ciphertext. `apps/web` invokes both startup stages and supplies its statically bundled Worker. The boot payload also delivers the structured index-injection table, and `applyIndexInjections` executes it before the shell entry runs. Script preload rows are advisory and skipped because `/plugins` resources resolve only through the tunnel; `loadBundle` fetches each combo on first demand, embeds its tunnel-only source map as a Base64 data URL, and executes the script as a Blob. The tunnel also exposes fetch-shaped transport and the API client.
...
- **Encrypted local-profile primitives remain inactive in the preview page**: the package has no user-facing first-run/migration UI or browser E2E for its origin registry, recovery import, and real PRF authenticator. `apps/web/src/preview.ts` deliberately supplies no `localProfile`, so this foundation cannot silently create or take over browser data. A post-boot IndexedDB write failure stops mirroring and preserves the live in-memory session; the page receives no live downgrade notification. Recovery packs are currently bounded to 256 MiB of serialized data and are intentionally refused instead of partially exported above that limit.
