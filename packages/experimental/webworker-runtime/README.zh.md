...
- **`lib/client.js`（页面半）**——启动分为相互独立的两段。`chooseWorkerHostSource({ image?, fixtureManifest? })` 可选地拥有 boot barrier 与 fixture manifest：没有 `preview-fixture` 时停在来源选择面板，合法 query 则直接选择；两条路径都返回按序排列的 overlays。`connectWorkerHost(worker, { image?, overlays?, localProfile? })` 仍是公开的基础运行态连接器；调用方跳过选择器时 overlay 列表为空。注册流程同时使用强制用户验证的 resident-passkey PRF 与版本化恢复口令包装同一个随机数据密钥，源站 registry 只存储严格校验后的 wrapper 元数据。registry、VFS key check 和加密 entries 共用同一个版本化 IndexedDB schema；新 profile 初始化会一次提交 registry record 与 key check。恢复导出在一致的完整 profile 快照外再加一层通过认证的 AES-GCM；恢复导入会先校验用户独立选择的 environment，再用一个 transaction 原子提交三个 store，且不会覆盖已有 profile。页面解锁所选本地 profile 后，把不可导出的 AES-GCM key 直接 structured-clone 给同源 Worker。Worker 请求持久存储与 profile 独占 Web Lock，只把 `home/` 和 `workspace/` 加密写入 IndexedDB，在不可变镜像/overlay 后恢复，并返回明确的 `durable` 或 `session_only` 启动状态。environmentId 与 UUID profileId 以密码学方式同时绑定两种 wrapper、profile key-check 和每条记录的密文。`apps/web` 调用这两段并提供静态打包的 Worker。boot 载荷还送达结构化 index 注入表，`applyIndexInjections` 在壳入口运行前逐行执行。脚本 preload 行只是提示，因此会被跳过：`/plugins` 资源只能经 tunnel 解析，`loadBundle` 会在首次需要时获取 combo、把仅 tunnel 可达的 sourcemap 内嵌为 Base64 data URL，再以 Blob 执行脚本。Tunnel 还暴露 fetch 形传输与 API 客户端。
...
- **preview 页面不启用加密本地 profile 原语**：本包尚无面向用户的首次使用/迁移 UI，也没有覆盖源站 registry、恢复导入与真实 PRF authenticator 的浏览器 E2E。`apps/web/src/preview.ts` 有意不传 `localProfile`，所以这层基础能力不会静默创建或接管浏览器数据。启动后的 IndexedDB 写入失败会停止镜像并保留当前内存会话；页面不会收到实时降级通知。恢复包当前以 256 MiB 序列化数据为上限，超过时会明确拒绝，不会生成不完整导出。
