...
- **preview 页面不启用加密本地 profile 原语**：本包没有同源 profile registry、恢复口令/恢复包导出流程或首次使用/迁移 UI。`apps/web/src/preview.ts` 有意不传 `localProfile`，所以这层基础能力不会静默创建或接管浏览器数据。启动后的 IndexedDB 写入失败会停止镜像并保留当前内存会话；页面不会收到实时降级通知。
