...
|---|---|---|---|
| 继承的进程环境 | `env` | 否 | 始终优先 |
| 受管凭据文档 | `file` | 是（`set`/`unset`） | 高于两个 `.env` 层 |
...
设置 `encryptionKeyFile` 后，上述 YAML 只存在于内存。磁盘保存规范的 `dsh-credentials-encrypted-v1:` 信封；每次写入都生成新的 96 位 nonce，并由 AES-256-GCM 认证。信封格式错误、内容被篡改或密钥不匹配都会在不引用明文的情况下关闭式失败。加密密钥与密文必须分别备份，但恢复时必须成对恢复。直接替换密钥文件并不等于轮换：停用旧密钥前，必须明确地重新加密文档，或让用户重新输入凭据。
...
未启用加密时，文档在 `0700` 目录下以 `0600` 权限存放，这挡得住其他 OS 用户，**挡不住**模型。工具进程（bash、文件系统工具）以同一用户身份运行，而已交付的 `workspace-write` 文件策略限制的是修改而非读取，因此它们读这个文件与读该用户拥有的任何其他文件毫无二致；也没有任何沙箱模式会把它单独挑出来。启用加密后，单独的文档已经无法利用，但能够读取所配置密钥文件的同 UID 进程仍可解密。harness 真正守住的更窄：它绝不把这两个解析后路径交给模型，也绝不把密钥或文档载入进程环境——这与用户的普通环境层 `$DSH_HOME/.env` 不同（见 [app-boot 的 Harness home 各层](../../boot/app-boot/README.zh.md#profiles)）。依赖这层分离的云部署还必须拒绝 agent 使用本地 Shell 和文件系统工具。
...
- **同 UID 进程若能同时读取文档与密钥，仍可还原凭据**——见[安全边界](#security-boundary)：认证加密保护持久化状态与备份，并不防御已经有权访问两份文件的代码；OS 钥匙串提供方仍是延后项。
- **密钥轮换必须显式执行**——只替换密钥文件会让现有文档不可读。保留旧密钥，直到文档已明确重新加密，或用户已重新输入凭据。
