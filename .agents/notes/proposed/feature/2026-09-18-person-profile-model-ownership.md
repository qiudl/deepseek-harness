...
The macOS Host control protocol now exposes recovery status and preimage restoration through that proof. It loads the global ledger lazily, so a damaged ledger makes recovery unavailable without preventing Host or unrelated Profiles from starting. Recovery does not read the old source and stays available even when source quiescence is not declared. The initial short-lived confirmation and write control methods, and the Windows recovery carrier, remain separate work.
