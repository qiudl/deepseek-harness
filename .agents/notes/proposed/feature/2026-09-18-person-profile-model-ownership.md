...
The macOS Host control protocol exposes recovery status and preimage restoration through that proof. It loads the global ledger lazily, so a damaged ledger makes recovery unavailable without preventing Host or unrelated Profiles from starting. Recovery does not read the old source and stays available even when source quiescence is not declared. Initial confirmation and write are advertised only after source quiescence is declared. Same-operation retry, the Windows claim carrier, and the Models UI remain separate work.
