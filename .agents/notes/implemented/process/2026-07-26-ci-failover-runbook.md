...
A successful runner allocation does not establish a passing fork gate. The Desktop Host package publication contract explicitly lists its broker and worker entrypoints and pinned Hub license/runtime files; new shipped files must be registered in `packageFileExtras` and kept in canonical manifest order. After removing invariant-only dependency wiring, regenerate the module graph rather than bypassing its freshness check.
