...
原基线记录为 fork master `dsh-v0.1.2-alpha.1`（0.1.2-alpha.1），上游为 上游 0.1.3-alpha.1 head 前后的 0.1.3-alpha.1 head；其 CI 规则要求使用输入 SHA，禁止移动 ref。这些是历史参照，不是当前运行时固定版本。
...
2026-09-07 的本地预研记录在 上游 0.1.3-alpha.1 head 上逐条独立试合 12 个内容提交：1 个成功，11 个冲突。冲突集中于 host/desktop-host、control-protocol、session-persistence-jsonl、core/session、core/agent-loop、apps/cli 和文档 i18n。原记录将主要工作归为语义适配（类 2）；全量 D1 报告仍按 REQ-20260907-0016 第 4 节执行。这些结果不能证明当前可合并性或测试状态。
