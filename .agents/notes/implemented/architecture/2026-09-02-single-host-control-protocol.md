...
`profile.remote_session` defines the wire values for a view-lease-bound Session and approval executor without creating a generic API proxy. Its closed command union carries a distinct UUID idempotency key and bounded operation-specific fields; its result admits only depth-, node-, string-, and frame-bounded JSON. The codec never accepts an HTTP path, Profile root, credential, browser cookie, or launch token. The Host now exposes an optional execution seam: it advertises the method only when an executor is installed, resolves the Profile from a live owner-bound lease, forwards connection cancellation, revalidates that lease after the awaited call, and passes the result through the canonical codec before responding. Startup composition still omits the capability until a Profile-worker executor is connected.
...
A Profile-contained plugin installation is not runtime compatibility evidence. Recovery permits a declared dependency only when its top-level module link resolves into the current packaged runtime or a content-addressed closure whose complete digest was verified; a flattened or copied `node_modules` tree is `compatibility_blocked` and remains available for read-only export instead of being started under a different runtime.
...
**Merge the old Profile directory into the newly created local Profile.** Session and plugin state have independent generations and ownership. Directory copying would make rollback and source ownership ambiguous, so the original Profile is opened in place and only its external runtime dependency closure is materialized privately.
...
Defining the remote Session values before advertising the operation lets both peers review and test one canonical command set, but no released caller can use the method until the Host executor is present. Capability negotiation, rather than decoder acceptance, remains the availability authority.
